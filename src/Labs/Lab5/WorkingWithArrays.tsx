...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <br />
      <hr />
      <h3>Updating Description from an Array</h3>
      <a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-primary float-end">
        Update Description
      </a>{' '}
      <input value={todo.description} className="form-control w-50 float-start me-2" onChange={(e) => setTodo({ ...todo, description: e.target.value })} />
      <br />
      <br />
      <hr />
      <h3>Updating Completed Field from an Array</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Completed
      </a>{' '}
      <input value={String(todo.completed)} className="form-control w-50 float-start" onChange={(e) => setTodo({ ...todo, completed: e.target.value === 'true' })} /> <br />
      <br />
      <hr />
      <br />
      <br />
